...
uploaded_file = st.file_uploader("Upload IR CSV file", type=["csv"])
        ...
        if show_peaks:
            for i in range(2, len(y_smooth)-2):
                if y_smooth[i] < y_smooth[i-1] and y_smooth[i] < y_smooth[i+1]:
                    if 2000 <= x[i] <= 2400:
                        continue  # ignore artifact region
                    ax.axvline(x[i], color="red", linestyle="--", alpha=0.6)
                    ax.text(x[i], y[i]-0.05, f"{int(x[i])}", color="blue", fontsize=8, rotation=90,
